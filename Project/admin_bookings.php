...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    $book_id = intval($_POST['booking_id'] ?? 0);
    $status = trim($_POST['status'] ?? '');
    $remarks = sanitizeInput($_POST['remarks'] ?? '');

    if ($book_id > 0 && in_array($status, ['pending', 'approved', 'completed', 'cancelled'])) {
        try {
            $sql = "UPDATE bookings SET status = ?, remarks = ? WHERE id = ?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$status, $remarks, $book_id]);
...
try {
    $sql = "SELECT b.*, u.fullname, u.phone FROM bookings b JOIN users u ON b.user_id = u.id ORDER BY b.event_date DESC";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
...
<body>
    ...
    <div class="container">
        ...
        <?php if (empty($bookings)): ?>
            <div class="empty">
                <h2>No Bookings</h2>
                <p>No facility bookings to review.</p>
            </div>
        <?php else: ?>
            <table>
                <tr>
                ...
                <tr>
                    <td>
                        <strong><?php echo htmlspecialchars($b['fullname']); ?></strong><br>
                        <small><?php echo htmlspecialchars($b['phone']); ?></small>
                    </td>
                    <td><?php echo htmlspecialchars($b['facility_type']); ?></td>
                    <td><?php echo htmlspecialchars($b['event_name']); ?></td>
                    <td>
                        <?php echo date('M d, Y', strtotime($b['event_date'])); ?><br>
                        <?php echo date('g:i A', strtotime($b['start_time'])); ?> - <?php echo date('g:i A', strtotime($b['end_time'])); ?>
                    </td>
                    <td><span class="badge <?php echo htmlspecialchars($b['status']); ?>"><?php echo ucfirst($b['status']); ?></span></td>
                    <td><button onclick="openModal(<?php echo $b['id']; ?>, '<?php echo htmlspecialchars($b['status']); ?>')">Review</button></td>
                </tr>
                <?php endforeach; ?>
            </table>
        <?php endif; ?>
    ...
    <div id="modal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <input type="hidden" name="booking_id" id="bookId">
